...
export const Route = createFileRoute("/certificate/$id")({
  head: ({ params }) => ({
    meta: [
      { title: `Sertifikat #${params.id} · Morrighans NFT` },
      { name: "description", content: "Sertifikat kepemilikan NFT modul ajar pendidik." },
    ],
  }),
  component: CertificatePage,
  notFoundComponent: () => (
    <div className="container mx-auto px-4 py-24 text-center">
      <h1 className="font-display text-2xl font-bold">Sertifikat tidak ditemukan</h1>
      <Link to="/gallery" className="mt-4 inline-block text-chain underline">Kembali ke galeri</Link>
    </div>
  ),
  errorComponent: ({ error, reset }) => (
    <div className="container mx-auto px-4 py-24 text-center">
      <h1 className="font-display text-2xl font-bold">Gagal memuat sertifikat</h1>
      <p className="mt-2 text-sm text-muted-foreground">{error.message}</p>
      <button onClick={reset} className="mt-4 text-chain underline">Coba lagi</button>
    </div>
  ),
});
...
function CertificatePage() {
  ...
  if (!loaded) return <div className="container mx-auto p-10 text-muted-foreground">Memuat…</div>;
  if (!m) {
    throw notFound();
  }
        ...
        <div className="relative grid gap-8 md:grid-cols-[1fr_auto] md:items-center">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-3 py-1 text-xs backdrop-blur">
              <BadgeCheck className="h-3 w-3 text-chain" /> Verified On-Chain
            </div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <span className="rounded-md bg-white/10 px-3 py-1 text-xs">{m.category}</span>
              <span className="rounded-md bg-white/10 px-3 py-1 text-xs font-mono">Token #{m.tokenId}</span>
              <span className="rounded-md bg-white/10 px-3 py-1 text-xs">Royalty {m.royaltyBps / 100}%</span>
            </div>
          </div>
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-3">
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-6 shadow-card-soft">
            <h3 className="font-display text-base font-semibold">Statistik</h3>
            <div className="mt-4 space-y-3 text-sm">
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">Unduhan</span>
                <span className="font-semibold">{m.downloads.toLocaleString("id-ID")}</span>
              </div>
              <div className="flex items-center justify-between">
          ...
          <div className="rounded-2xl border border-border bg-card p-6 shadow-card-soft">
            <h3 className="font-display text-base font-semibold">Verifikasi</h3>
            ...
            <Button variant="outline" className="mt-2 w-full">
              <Download className="mr-2 h-4 w-4" /> Unduh Modul
            </Button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col gap-1 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
      ...
      <div className="flex min-w-0 items-center gap-2">
        ...
        {onCopy && (
          <button onClick={onCopy} className="rounded p-1 text-muted-foreground hover:bg-secondary hover:text-foreground" aria-label="Salin">
            {copied ? <Check className="h-3.5 w-3.5 text-success" /> : <Copy className="h-3.5 w-3.5" />}
          </button>
        )}
        {external && (
          <a href={external} target="_blank" rel="noreferrer" className="rounded p-1 text-muted-foreground hover:bg-secondary hover:text-foreground" aria-label="Buka">
